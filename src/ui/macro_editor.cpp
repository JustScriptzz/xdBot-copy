...
bool MacroEditLayer::init() {
    if (!Popup::init(438, 247, Utils::getTexture().c_str()))
        return false;
...
void MacroEditLayer::loadPage(int page) {
    ...
    pageLeftBtn->getChildByType<CCSprite>(0)->setOpacity(
        (disableArrows && !empty) || inputs.empty() ? 130 : 255);
...
void MacroEditLayer::mergeMacro(std::vector<input> mergeInputs, bool players[2],
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = mergeInputs.size() - 1; j >= 0; j--) {
            ...
            if (static_cast<int>(inp.player2) != i)
                continue;
